...
export function Header() {
  ...
  const suggestContent = (
    <div className="absolute right-0 top-full mt-1 z-50 max-h-80 w-full overflow-auto rounded-2xl border border-slate-200 bg-white p-2 text-right shadow-xl">
      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex h-16 items-center gap-3">
          ...
          <Link href="/" className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[10px] text-emerald-600 font-medium">همه چیز برای همه</p>
            </div>
          </Link>

          {/* Desktop Search */}
          <form ref={searchRef} onSubmit={submitSearch} className="relative hidden flex-1 md:block">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            <Link href="/products" className="rounded-lg px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-900">
              محصولات
            </Link>
            {user ? (
              <Link href="/orders" className="rounded-lg px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-900">
                سفارش‌ها
              </Link>
            ) : null}
          </nav>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 transition hover:bg-slate-50 md:hidden"
            >
            ...
            {/* Cart */}
            <Link href="/cart" className="relative flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 transition hover:bg-slate-50" aria-label="سبد خرید">
            ...
            {/* User */}
            {user ? (
              <Link href="/profile" className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-600 transition hover:bg-slate-50" aria-label="پروفایل">
                <UserRound className="h-5 w-5" />
              </Link>
            ) : (
              <Link href="/login">
                <button className="flex h-9 items-center gap-1.5 rounded-lg bg-emerald-500 px-4 text-xs font-bold text-white shadow-sm shadow-emerald-200 transition hover:bg-emerald-600">
                  ورود
                </button>
              </Link>
            )}
        ...
        <div className="hidden lg:flex items-center gap-4 border-t border-slate-50 py-2 -mx-4 px-4 overflow-x-auto scrollbar-hide">
          {[
            ...
            <span key={text} className="shrink-0 text-[11px] text-slate-400 font-medium">{text}</span>
          ))}
        </div>
      </div>
          ...
          >
            ...
            {query.trim().length >= 2 && (
              <div className="mt-2 max-h-72 overflow-auto rounded-xl border border-slate-200 bg-white">
                {suggestions.isLoading && <p className="p-3 text-sm text-slate-500">در حال جستجو...</p>}
                {!suggestions.isLoading && (suggestions.data ?? []).map((item) => (
                  <Link
                    key={item.id}
                    href={`/products/${item.id}`}
                  ...
                  >
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-slate-100">
                      {item.images?.[0] ? <img src={item.images[0]} alt="" className="h-full w-full object-cover" /> : <span className="text-sm">🛍️</span>}
                    </div>
                    <div className="min-w-0 flex-1">
                ...
                {!suggestions.isLoading && suggestions.data?.length === 0 && <p className="p-3 text-sm text-slate-500">نتیجه‌ای یافت نشد.</p>}
              </div>
            )}
          </motion.div>
        )}

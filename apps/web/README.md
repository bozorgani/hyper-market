...
| `CSP_MODE` | `src/middleware.ts` | Yes in production policy | `report-only` during a controlled CSP rollout; `enforce` after violations have been reviewed. The current application default is `report-only`, so production deployment must set this intentionally. |
...
The current CSP is generated by `src/middleware.ts` with a per-request nonce. It is intentionally report-only by default while violations are reviewed.
